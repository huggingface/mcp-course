...
Next, we'll login to HF Hub and then connect to the MCP Server and get the tools that we can use to answer questions.

```python
# login if running MCP Client locally. This is not needed if deploying the app on HF Hub
login("<your_token_goes_here>")
