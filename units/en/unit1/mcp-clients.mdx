...
```
(While completing course in 2026) I a get an error while running the huggingface-cli command: The term 'huggingface-cli' is not recognized as the name of a cmdlet, function, script file, or operable program.
The command that I used - obtained from https://huggingface.co/docs/huggingface_hub/package_reference/cli - 
```bash
hf auth login
```
- allowed me to login.
...
The model throws a Value error while running the agent.
ValueError: Model Qwen/Qwen2.5-72B-Instruct is not supported by provider nebius.
Qwen/Qwen3-32B - still supported by nebius.
(I apologize if this is not the right way to make commits, I have never done this before)
