...
MCP 服务器通过通信协议向客户端公开各种能力。这些能力主要分为四大类，每类都有独特的特征和使用场景。让我们探索这些构成 MCP 功能基础的核心原语。

> [!TIP]
> 在本节中，我们将以每种语言中与框架无关的函数形式展示示例。这是为了专注于概念及其如何协同工作，而不是任何框架的复杂性。
>
...
- **控制权**：工具通常是**模型控制的**，这意味着 AI 模型（LLM）根据用户的请求和上下文决定何时调用它们。
- **安全性**：由于工具能够执行具有副作用的操作，工具执行可能存在危险。因此，它们通常需要明确的用户批准。
...
- **控制权**：资源是**应用程序控制的**，这意味着宿主应用程序通常决定何时访问它们。
...
提示词是预定义的模板或工作流，用于指导用户、AI 模型和服务器能力之间的交互。

- **控制权**：提示词是**用户控制的**，通常在宿主应用程序的 UI 中作为选项呈现。
- **目的**：它们为优化使用可用工具和资源而构建交互结构。
- **选择**：用户通常在 AI 模型开始处理之前选择一个提示词，为交互设置上下文。
- **使用场景**：常见工作流、专业任务模板、引导式交互。
...
def code_review(code: str, language: str) -> list:
    ...
    return [
        {
            "role": "system",
            "content": f"你是一名代码审查员，正在检查 {language} 代码。请提供详细的审查，突出最佳实践、潜在问题和改进建议。"
        },
        {
            "role": "user",
...
function codeReview(code, language) {
    return [
        {
            role: 'system',
            content: `你是一名代码审查员，正在检查 ${language} 代码。请提供详细的审查，突出最佳实践、潜在问题和改进建议。`
        },
        {
            role: 'user',
...
采样允许服务器请求客户端（特别是宿主应用程序）执行 LLM 交互。

- **控制权**：采样是**服务器发起的**，但需要客户端/宿主的协助。
- **目的**：它使服务器驱动的代理行为以及潜在的递归或多步骤交互成为可能。
- **安全性**：与工具类似，采样操作通常需要用户批准。
- **使用场景**：复杂的多步骤任务、自主代理工作流、交互式流程。
...
> [!TIP]
> 这种人在回路（human-in-the-loop）的设计确保用户保持对 LLM 看到和生成内容的控制。在实现采样时，提供清晰、结构良好的提示词并包含相关上下文非常重要。

## 能力如何协同工作

让我们看看这些能力如何协同工作以实现复杂的交互。在下表中，我们概述了各种能力、谁控制它们、控制方向以及其他一些细节。

| 能力 | 控制者 | 方向 | 副作用 | 需要批准 | 典型使用场景 |
|------------|---------------|-----------|--------------|-----------------|-------------------|
| 工具      | 模型（LLM）   | 客户端 → 服务器 | 是（可能） | 是 | 操作、API 调用、数据操作 |
...
MCP 的关键特性之一是动态能力发现。当客户端连接到服务器时，它可以通过特定的列表方法查询可用的工具、资源和提示词：
