...
MCP 定义了一个标准化的通信协议，使客户端和服务器能够以一致、可预测的方式交换消息。这种标准化对于社区的互操作性至关重要。在本节中，我们将探讨 MCP 中使用的协议结构和传输机制。

> [!WARNING]
> 我们将深入探讨 MCP 协议的细节。你不需要了解所有这些内容就能使用 MCP 进行构建，但了解它的存在和工作原理是有益的。

## JSON-RPC：基础协议

MCP 的核心使用 **JSON-RPC 2.0** 作为客户端和服务器之间所有通信的消息格式。JSON-RPC 是一种用 JSON 编码的轻量级远程过程调用协议，它具有以下特点：
...
宿主应用程序将服务器作为子进程启动，并通过写入其标准输入（stdin）和从其标准输出（stdout）读取来与其通信。

> [!TIP]
> 此传输的**使用场景**包括本地工具，如文件系统访问或运行本地脚本。

此传输的主要**优势**是简单、无需网络配置，并且由操作系统安全地沙箱化。
...
> [!TIP]
> 此传输的**使用场景**包括连接到远程 API、云服务或共享资源。

此传输的主要**优势**是可以跨网络工作、支持与 Web 服务集成，并且与无服务器环境兼容。

MCP 标准的最新更新引入或完善了"可流式 HTTP"，它通过允许服务器在需要时动态升级到 SSE 进行流式传输，同时保持与无服务器环境的兼容性，提供了更大的灵活性。
...
MCP 协议被设计为可扩展和可适应的。初始化阶段包括版本协商，允许在协议演进时保持向后兼容性。此外，功能发现使客户端能够适应每个服务器提供的特定功能，从而在同一生态系统中实现基础服务器和高级服务器的混合使用。